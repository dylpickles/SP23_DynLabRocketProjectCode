...
delta_t = 0.3; % need to work on getting this down

%Setting initial conditions on the persistent arrays
P_int = input("What was the launch pressure in psi? ")*6.89476;      %internal pressure [kPa], converted from psi
v_rocket = 0;   %velocity of the rocket relative to a fixed frame [m/s]
h = 0;          %height [m]
V_water = input("What was the water volume in mL? ")/1000000; %for the test flight we care about
...
%arrays for the values that we're looking to keep track of
...
totalTime = 0; %total elapsed time
...
figure;
plot(tARRAY, hARRAY*3.28);
title('Cool Rocket Simulation in Freedom Units');
xlabel('Time (s)');
ylabel('Position (ft)');


max_height = max(hARRAY)*3.28 %max height in feet
